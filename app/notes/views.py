...
def edit_note(request, id):
    ...
    if note.is_encrypted:
        messages.add_message(
            request, messages.ERROR, "You cannot edit encrypted notes. Try adding new one.")
        return redirect(f"/notes/{id}/show")

    if request.method == 'GET':
        form = NoteForm(
            initial={"content": note.content, "is_public": note.is_public})
...
def process_encrypted(request, data):
    ...
    if "is_public" in data and data["is_public"] == ["on"]:
        messages.add_message(request, messages.ERROR,
                             "You cannot share encrypted notes!")
        return False
